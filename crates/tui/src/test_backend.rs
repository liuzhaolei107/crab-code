//! `vt100`-driven backend used in TUI integration tests.
//!
//! Wraps a `CrosstermBackend` whose underlying writer is a `vt100::Parser`,
//! so we can run the real rendering pipeline (including ANSI escape codes
//! emitted by `insert_history`) and inspect the resulting screen contents
//! cell-by-cell instead of guessing at byte sequences.
